...
1. **Mem0** — user preference / fact memory, multi-user via `user_id` namespacing. Correct as a practical characterization: Mem0 stores extracted facts per `user_id` with semantic search, and it does track `created_at`/`updated_at` plus a change-history log — but it has no bi-temporal reasoning (no "this fact was true from X to Y, then superseded by Z"). Minor nuance: Mem0 also offers an optional graph-memory backend (Neo4j-based), so "flat fact storage" describes its default/primary mode, not the only mode. Directionally accurate for this repo's purposes.
2. **Zep** — knowledge graph + timestamps for complex, evolving data relationships. Multi-user **and** time-aware natively — matches what's already seen in zep-project, where Graphiti's bi-temporal model (`valid_at`/`invalid_at`) is exactly this. This is the direct fit for "each user, own long-term, time-related info": Zep gives you both axes (multi-user isolation + temporal fact reasoning) natively, so it's a better direct fit than combining Mem0 with something else to bolt on the temporal part.
...
3. **Letta** — long-running autonomous agent framework. Correct to skip for a memory-layer-only comparison: Letta (ex-MemGPT) ships its own agent runtime/server with self-editing memory blocks baked into its execution loop — adopting it means adopting their agent model, not just calling a memory API from an existing agent loop. A much bigger adoption decision than Mem0/Zep.
4. **LangMem** — procedural memory (the agent rewrites its own system prompt based on feedback). Correct on the procedural-memory description; slightly more nuanced on the coupling claim: LangMem's memory managers are somewhat usable standalone, but it's built on LangGraph's `BaseStore` and lives in the LangChain org, so in practice adopting it still pulls in that ecosystem. The "don't want to lock into LangChain" concern is a reasonable practical read even if not a hard technical requirement — skipped for now on that basis.
